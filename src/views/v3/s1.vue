...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(pn) {
      ...
      if (this.selectDisabled) {
        ...
        model
          .getEduByCategoryId(this.optionsValue, this.pn, this.ps)
          .then((res) => {
            this.tableData = res.rows;
            this.total = res.total;
          });
    ...
    handleSizeChange(ps) {
      this.ps = ps;
      model.getEduByCategoryId(this.optionsValue, 1, this.ps).then((res) => {
        this.tableData = res.rows;
        this.total = res.total;
      });
    },
  ...
  mounted() {
    model
      ...
      .then((typeId) => {
        // 根据类型获取数据
        model.getEduByCategoryId(typeId, 1, this.ps).then((res) => {
          this.tableData = res.rows;
          this.total = res.total;
        });
      });
