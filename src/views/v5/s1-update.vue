<template>
  <div class="root">
    <div class="title">
      ...
      <h2>审核<span>-</span>帖子id为：<span>{{ id }}</span> 的详细信息</h2>
    </div>
    <main>
      <el-row>
        <el-col :span="16">
          <el-row class="txt-row" v-for="(value, name, index) in label" :key="index">
            <el-col style="text-align:right" v-if="name!='updateBy' && name != 'updateDate' && name != 'state'" class="txt-key" :span="3">{{ value }}：</el-col>
            <el-col v-if="name!='updateBy' && name != 'updateDate' && name != 'state' && name != 'images'" class="txt-value"  :span="21">{{ data[name] }}</el-col>
...
export default {
  ...
  methods: {
    ...
    back() {
      this.$router.push("/v5/s1?state=0");
    },
  },
  mounted() {
