...
module.exports = class {
  ...
  static list(page, size) {
    return new Promise((resolve) => {
      zp_axios.get(`/technicalArticles/findAll/${page}/${size}`).then((res) => {
        res = res.data.data;
  ...
  static update(obj, id) {
    return new Promise((resolve) => {
      ...
      console.log(obj);
      zp_axios
        .put("/technicalArticles/update/" + id, obj)
        .then((res) => {
  ...
  static add(obj) {
    return new Promise((resolve) => {
      ...
      obj.updateDate = new Date();
      zp_axios
        .post("/technicalArticles/add/", obj)
        .then((res) => {
          // console.log('res: ', res);
          resolve(res.data.code);
  ...
  static deletePicOrWordOrPdf(url) {
    return new Promise((resolve) => {
      axios
  ...
  static count() {
    return new Promise((resolve) => {
      zp_axios
        .get("/technicalArticles")
        .then((res) => {
          resolve(res.data.data.length);
        });
    });
  }
  static getTechnicalArticlesTypes() {
    return new Promise((resolve) => {
      zp_axios
        .get("/technicalArticlesTypes")
        .then((res) => {
          resolve(res.data.data);
        });
    });
  ...
  static getByKeyWord(value, pn, ps) {
    ...
    return new Promise((resolve) => {
      axios.get(`http://106.75.154.40:9010/education/search/time/${pn}/${ps}/0?key=${value}`)
        .then((res) => {
          res = res.data.data;
          res.rows.forEach(item => {
            item["createDate"] = this.formatTime(item["createDate"]);
   ...
   //根据渔技文章类型获取资讯
  static getTechnicalArticlesByCategoryId(categoryId, pn, ps) {
    return new Promise((resolve) => {
      zp_axios.get('/technicalArticles/search/searchByTypeId/' + categoryId + '/' + pn + '/' + ps)
        .then((res) => {
          res = res.data.data;
          res.rows.forEach(item => {
            item["createDate"] = this.formatTime(item["createDate"]);
  ...
  static findDeleteRecord(pn, ps) { 
    return new Promise((resolve) => {
      zp_axios.get('/technicalArticles/findByDelete/' + pn + '/' + ps)
        .then((res) => {
          res = res.data.data;
          res.rows.forEach(item => {
            item["createDate"] = this.formatTime(item["createDate"]);
